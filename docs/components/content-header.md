...
<header class="content-article__header">
  <div class="content-article__header-badge">
    {% if "/reading/" in page.url %}
      <span class="content-article__header-tag content-article__header-tag--reading">
        <span class="content-article__header-tag-icon" aria-hidden="true">📘</span>読んで学ぶ
      </span>
    {% else %}
      <span class="content-article__header-tag content-article__header-tag--handson">
        <span class="content-article__header-tag-icon" aria-hidden="true">🛠️</span>触って学ぶ
      </span>
    {% endif %}
  </div>
...
- **バッジタグ (`.content-article__header-tag`)**:
  - 表示中の教材が「読んで学ぶ」（`/reading/`）か「触って学ぶ」（`/hands-on/`）かをURLから動的に判定し、対応する絵文字アイコンとラベルテキストを表示します。
