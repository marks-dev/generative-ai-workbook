...
- 教材詳細（末端）ページのメインコンテンツにおけるメディア掲載に関する厳格な共通ガイドラインを策定：
  - 画像はリポジトリの容量逼迫を防止するためWebP形式（`.webp`）に完全統一し、`assets/images/`配下に格納
  - スライドは外部依存の404エラーおよびアクセス拒否を回避しコンテンツを一元管理するため、外部iframe埋め込みを全面禁止し、`assets/slides/`配下にHTML形式のスライドモックを作成・配置してiframe表示する方式に統一
  - YouTube動画は将来的なリンク切れを考慮しiframe直接埋め込みを廃止、専用注記（「本件に関連した公開YouTube動画のURLを参考までに掲載します...」）を添えた別タブ表示の外部リンク化に統一
  - 日本人学習者を対象とすることを考慮し、画像・スライド・動画は可能な限り日本語アセットを選定・採用する方針を確定
  - パススルーコピーされるHTMLスライドにおけるフロントマター露出の不具合を防止するため、アセットHTMLへのフロントマター記載を全面禁止（純粋な静的HTMLとして配置）
